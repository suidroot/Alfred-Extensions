...
WORKINGPATH="~/Library/Caches/com.runningwithcrayons.Alfred-3/Workflow\ Data/dk.aiyo.VirtualBoxControl"
...
if [ -z "$1" ] || [ $(stat -f "%m" '$WORKINGPATH/vms') -ge $TIME ]
   then
   # cache the VMs
    if [ ! -d '$WORKINGPATH/' ]
        then
    ...
    VBoxManage list vms -l | sed -n -e '/^Name/N' -e '/\nGroups/P' -e '/\nGuest OS/p' -e '/^Guest OS/P' -e '/^UUID/P' -e '/^State/P' > '$WORKINGPATH/vms'
fi
...
if [[ $(echo "$1" | grep -c " >" ) -gt 0 ]]
    ...
    VMINFO=$(grep -A 3 "$NAME$" '$WORKINGPATH/vms')
    if [[ ! -z "$VMINFO" ]]
        then
